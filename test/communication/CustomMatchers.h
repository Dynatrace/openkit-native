...
{
	if (arg->getStateType() == communication::AbstractBeaconSendingState::StateType::BEACON_SENDING_INIT_STATE)
	{
		return true;
	}
...
{
	if (arg->getStateType() == communication::AbstractBeaconSendingState::StateType::BEACON_SENDING_CAPTURE_ON_STATE)
	{
		return true;
	}
	*result_listener << "Unexpected stateType " << as_integer(arg->getStateType());
...
{
	if (arg->getStateType() == communication::AbstractBeaconSendingState::StateType::BEACON_SENDING_CAPTURE_OFF_STATE)
	{
		return true;
	}
	*result_listener << "Unexpected stateType " << as_integer(arg->getStateType());
