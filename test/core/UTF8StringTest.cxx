...
TEST_F(UTF8StringTest, UrlEncodeQueryParameterWithSpacesAndEqualsSign)
{
	UTF8String s("q=greater than 5");
	UTF8String expectation("q%3Dgreater%20than%205");

	UTF8String encoded = s.urlencode();
	EXPECT_TRUE(encoded.equals(expectation));
}

TEST_F(UTF8StringTest, UrlEncodeQueryParameterWithSpacesAndEqualsSignFinallyDecodeAgain)
{
	UTF8String s("q=greater than 5");
	UTF8String encoded = s.urlencode();
	UTF8String decoded = encoded.urldecode();

	EXPECT_TRUE(decoded.equals(s));
}

TEST_F(UTF8StringTest, UrlEncodeStringNotChangedAllCharactersAllowed)
{
...
TEST_F(UTF8StringTest, UrlEncodeUTF8MultibyteName)
{
	UTF8String s("\xD7\xAA\xf0\x9f\x98\x8b");
...
TEST_F(UTF8StringTest, UrlEncodeUTF8MultibyteNameFinallyDecodeAgain)
{
	UTF8String s("\xD7\xAA\xf0\x9f\x98\x8b");
	UTF8String encoded = s.urlencode();
	UTF8String decoded = encoded.urldecode();
...
TEST_F(UTF8StringTest, UrlDecodeFailing_PercentFollowedByNonHexCharacterTwoInvalidBytes)
{
	UTF8String s("invalid%string");
	UTF8String decoded = s.urldecode();

	UTF8String expectation("invalid?string");
	EXPECT_TRUE(decoded.equals(expectation));
}

TEST_F(UTF8StringTest, UrlDecodeFailing_PercentFollowedByNonHexCharacterSecondByteInvalid)
{
...
TEST_F(UTF8StringTest, UrlDecodeFailing_PercentFollowedByNonHexCharacterFirstByteInvalid)
{
...
TEST_F(UTF8StringTest, UrlDecodeFailing_OneCharacterMissingAtTheEnd)
{
...
TEST_F(UTF8StringTest, UrlDecodeFailing_BothCharactersMissingAtTheEnd)
{
...
TEST_F(UTF8StringTest, UrlEncodeDecodePercentSignPreserved)
{
